...
---
# Source code
**Source Code:** [vitalLink](vitalink_project/app.py)
...
- **Python Version**: Python 3.8 or higher (tested on 3.9–3.12)
- **Operating System**: Windows, macOS, Linux (any platform that supports Streamlit)
- **Browser**: Modern browser with full WebRTC support (Google Chrome or Firefox recommended)
- **Internet Connection**: Required for live video consultation (uses external signaling and TURN servers)
- **Dependencies** (listed in `requirements.txt` format):
  ```txt
  streamlit>=1.28.0
  pandas>=2.0.0
  streamlit-cookies-controller>=0.0.5
- No additional environment variables are required.
- **Hardware** : Any device capable of running Streamlit (CPU + camera for video calls)
---
#  Declaration

**This project is developed purely for demonstration, educational, and portfolio purposes only.**

- VitalLink is **NOT** a certified medical device and must **NOT** be used for actual clinical practice, diagnosis, treatment, or real patient care.
- All health alerts, vital sign interpretations, and prescriptions generated by the system are rule-based examples and do not replace professional medical advice.
- The developer and this application assume **no medical, legal, or professional liability** for any use of the software.
- Patient data (vitals, history, video) stored in this demo is for testing purposes only and is kept locally in `vitalink.db`.
- Video consultation functionality is provided strictly as a technical demonstration.
- By using, viewing, or modifying this project, you acknowledge that it is a prototype and not intended for real-world medical deployment.
